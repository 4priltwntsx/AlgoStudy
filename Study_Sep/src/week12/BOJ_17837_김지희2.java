		...
		while (true) {
			...
			for (int k = 1; k <= K; k++) {// 1번부터 K번 말 이동
				...
				if ((ni > 0 && nj > 0 && ni <= N && nj <= N) && map[ni][nj] == 0) {// 흰색인 경우에는 그 칸으로 이동한다!!!! 이미 말이 존재하면
					...
					for (int m : movelist) {
						board[chessman[m].i][chessman[m].j].remove((Integer) m);
						chessman[m].i = ni;
						chessman[m].j = nj;
					}
				}

				else if ((ni > 0 && nj > 0 && ni <= N && nj <= N) && map[ni][nj] == 1) {// 빨간색인 경우에는 이동한 후에 순서가 reverse
																						// 돼서 추가된다.
					ArrayList<Integer> movelist = new ArrayList<>();

					for (int j = board[cur.i][cur.j].size() - 1; j >= 0; j--) {
						int n = board[cur.i][cur.j].get(j);
						board[ni][nj].add(n);
						movelist.add(n);
